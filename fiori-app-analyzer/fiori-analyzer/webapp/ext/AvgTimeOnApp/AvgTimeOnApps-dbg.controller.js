sap.ui.define([
	"sap/ui/model/json/JSONModel"
], function (JSONModel) {
	"use strict";

	return {

		onInit: function () {
			var that = this;
			var oView = that.getView();

			// NOTE: This card uses hardcoded mock data — not wired to any OData entity set
			var data = {
				"apps": [{
					"Appname": "Rebates App",
					"users": 455,
					"Time": 60
				}, {
					"Appname": "Agreements App",
					"users": 350,
					"Time": 45
				}, {
					"Appname": "Approval App",
					"users": 300,
					"Time": 35
				}, {
					"Appname": "Route fiorimoni App",
					"users": "253",
					"Time": 30
				}, {
					"Appname": "Rebate Settlement App",
					"users": "100",
					"Time": 25
				}, {
					"Appname": "OPA App",
					"users": "120",
					"Time": 20
				}, {
					"Appname": "User Report App",
					"users": "80",
					"Time": 10
				}],
				"Errors": [{
					"Error": "Authentication Error"
				}, {
					"Error": "sap is not defined"
				}, {
					"Error": "failed to load from: SyntaxError: Unexpected token )"
				}, {
					"Error": "Controller couldn\u0092t be instantiated"
				}],
				"TopUsers": [{
					"User": "Ajay"
				}, {
					"User": "Gavin"
				}, {
					"User": "Shinjan"
				}, {
					"User": "jonathanbragg"
				}]
			};

			var oUserPerAppModel = new JSONModel(data);
			oView.setModel(oUserPerAppModel, "oUserPerAppModel");

			var oAvgTimeperApp = that.oAvgTimeperApp = oView.byId("idAvgTimeOnAppsViz");
			oAvgTimeperApp.setVizProperties({
				plotArea: {
					dataLabel: {
						visible: true
					}
				},
				valueAxis: {
					label: {},
					title: {
						visible: false
					}
				},
				categoryAxis: {
					title: {
						visible: false
					}
				},
				title: {
					visible: false
				}
			});
			that.byId("idAvgTimeOnAppsViz").setLegendVisible(false);
		},

		detailNav: function () {
		}

	};
});
