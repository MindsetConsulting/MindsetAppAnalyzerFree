	...
	return Component.extend("com.mindset.flpext.Component", {
		...
		init: function () {
			...
			$(document).ready(function() {
				$(document).on('click', '.sapFGridContainerItemInnerWrapper', function(e) {
					if(!me.tileClicked){
						me.tileClicked = true;
						hash = e.target.closest("a").getAttribute('href');
						oSementicObject = hash.split('#')[1].split('?')[0].split('-')[0];
						oAction = hash.split('#')[1].split('?')[0].split('-')[1];
						var appUrl = e.currentTarget.baseURI;
						var userId = me.userId;
						//sAppDesc = e.currentTarget.textContent;
						sAppDesc = e.target.closest("a").getAttribute('title');
						if(sAppDesc){
							sAppDesc = sAppDesc.split('\n')[0];
						}
						me.onCallAppLogInSet(oSementicObject, oAction, appUrl, userId, sAppDesc, browser, Device, os, latitude, longitude);
					}
				});
			
				$(document).on('click', '.sapFGridContainerItemWrapper', function(e) {
					if(!me.tileClicked){
						me.tileClicked = true;
						hash = e.target.closest("a").getAttribute('href');
						oSementicObject = hash.split('#')[1].split('?')[0].split('-')[0];
