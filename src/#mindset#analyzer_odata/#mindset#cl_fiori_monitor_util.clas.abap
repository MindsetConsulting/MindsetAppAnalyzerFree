...
public section.
  ...
  types:
    ty_device_user_t TYPE STANDARD TABLE OF /mindset/cl_fiori_moni_mpc_ext=>ts_flpdevice WITH DEFAULT KEY .
  types:
    ty_browser_user_t TYPE STANDARD TABLE OF /mindset/cl_fiori_moni_mpc_ext=>ts_flpbrowser WITH DEFAULT KEY .
  types:
    ty_geo_login_t TYPE STANDARD TABLE OF /mindset/cl_fiori_moni_mpc_ext=>ts_geologin WITH DEFAULT KEY .
  types TY_AVG_LOAD_TIME type /MINDSET/APPINFO-LOAD_TIME .
  ...
  methods GET_BROWSER_USERS
    returning
      value(RT_LOGINS) type TY_BROWSER_USER_T .
  methods GET_DEVICE_USERS
    returning
      value(RT_LOGINS) type TY_DEVICE_USER_T .
  methods CONSTRUCTOR .
...
*      pageviews
  ...
  METHOD get_browser_users.

    DATA: ls_logins TYPE LINE OF ty_browser_user_t.
    ...
    LOOP AT lt_flpinfo INTO DATA(ls_flpinfo).
      ...
      ls_logins-user_id = ls_flpinfo-user_id.
      ls_logins-browser = ls_flpinfo-browser.
      ls_logins-log_time = ls_flpinfo-log_time.
  ...
  METHOD get_device_users.

    DATA: ls_logins TYPE LINE OF ty_device_user_t.
    ...
    LOOP AT lt_flpinfo INTO DATA(ls_flpinfo).
      ...
      ls_logins-user_id = ls_flpinfo-user_id.
      ls_logins-device_type = ls_flpinfo-device_type.
      ls_logins-log_time = ls_flpinfo-log_time.
